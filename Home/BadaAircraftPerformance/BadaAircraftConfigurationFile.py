...
from Home.Config.BadaAircraftData import BadaInfo
...
class AircraftConfiguration(FlightEnvelope):
    ...
    def computeThrustNewtons(self, geopotentialPressureAltitudeFeet ):
        ...
        RotorRadius = 2.0
        # 旋翼旋转速度(角速度)
        RotorAngularVelocity = 300.0
        # 空气密度
        AirDensity = 0.1
        # 旋翼拉力系数
        RotorThrustCoefficient = 0.75
        # 旋翼拉力
        RotorThrust = 0.5 * AirDensity * RotorThrustCoefficient * RotorAngularVelocity * RotorAngularVelocity * RotorRadius * RotorRadius * RotorRadius * math.pi
        thrustNewtons = RotorThrust
        #zhd 2021/11/28
...
#             self.setCruiseConfiguration(elapsedTimeSeconds)
    ...
    # zhd 需要一个存储全局变量的类，存储竖直方向的速度
    trueAirSpeedAtAltMetersSecond = 0.0
    def fly(self, 
            elapsedTimeSeconds, 
            deltaTimeSeconds, 
            aircraftMassKilograms, 
        ...
        if self.isDepartureGroundRun():
            ...
            #zhd当做是垂直起飞
            #螺旋桨的前置角度
            liftNewtons = thrustNewtons

            aircraftAccelerationAtAlt = liftNewtons / aircraftMassKilograms - gravityCenter

            self.trueAirSpeedAtAltMetersSecond += aircraftAccelerationAtAlt * deltaTimeSeconds
            trueAirSpeedMetersSecond += aircraftAccelerationAtAlt * deltaTimeSeconds

            deltaDistanceMeters = 0.0
            #deltaAltitudeMeters = self.trueAirSpeedAtAltMetersSecond * deltaTimeSeconds
            deltaAltitudeMeters = trueAirSpeedMetersSecond * deltaTimeSeconds
            altitudeMeanSeaLevelMeters += deltaAltitudeMeters
            flightPathAngleDegrees = 5.0

#wl 改成比起点海拔高40m之后跳转altitudesum
            if altitudeMeanSeaLevelMeters - self.getDepartureAirportAltitudeMSLmeters() > self.aircraftInfo.getUpHeight():
            #if (altitudeMeanSeaLevelMeters > altitudesum):
            ...
            #垂直起飞之后向前向上飞
            RotorAngle = 18
            RotorAngleRad = math.radians(RotorAngle)
            liftNewtons = thrustNewtons * math.cos(RotorAngleRad)

            aircraftAccelerationAtAlt = liftNewtons / aircraftMassKilograms - gravityCenter
            aircraftAcceleration = thrustNewtons / aircraftMassKilograms

            self.trueAirSpeedAtAltMetersSecond += aircraftAccelerationAtAlt * deltaTimeSeconds
            trueAirSpeedMetersSecond += aircraftAcceleration * deltaTimeSeconds * 0.21
            # zhd 如果速度到达最大速度，速度等于最大速度
            mach = self.atmosphere.tas2mach(tas=trueAirSpeedMetersSecond,
                                            altitude=altitudeMeanSeaLevelMeters,
                                            alt_units='m',
                                            speed_units='m/s')
            targrtmach = self.getTargetCruiseMach()
            if(mach >= targrtmach + 0.0001):
                #zhd暂定超出0.3马赫后速度恒定为101m/s，即365km/h，0.3马赫，需修改
                trueAirSpeedMetersSecond = 101


            deltaDistanceMeters = trueAirSpeedMetersSecond * math.sin(RotorAngleRad) * deltaTimeSeconds
            deltaAltitudeMeters = self.trueAirSpeedAtAltMetersSecond * deltaTimeSeconds
            altitudeMeanSeaLevelMeters += deltaAltitudeMeters
            flightPathAngleDegrees = 5.0
#wl 改为比起始点海拔高400m
            if (self.getCurrentAltitudeSeaLevelMeters() - self.getDepartureAirportAltitudeMSLmeters() >= self.aircraftInfo.getCruiseHeight()):
            #if (self.getCurrentAltitudeSeaLevelMeters() >= 700.0):
            ...
            self.trueAirSpeedAtAltMetersSecond += aircraftAccelerationAtAlt * deltaTimeSeconds
            trueAirSpeedMetersSecond += aircraftAcceleration * deltaTimeSeconds * 0.5
            mach = self.atmosphere.tas2mach(tas=trueAirSpeedMetersSecond,
                                            altitude=altitudeMeanSeaLevelMeters,
                                            alt_units='m',
                                            speed_units='m/s')
            ...
            altitudeMeanSeaLevelMeters += deltaAltitudeMeters
            #zhd
            if (altitudeMeanSeaLevelMeters >= 5000.0):
                altitudeMeanSeaLevelMeters = 5000.0
            flightPathAngleDegrees = - 5.0
            if (self.trueAirSpeedAtAltMetersSecond <= 0.0001):
                self.setCruiseConfiguration(elapsedTimeSeconds + deltaTimeSeconds)
            if (distanceStillToFlyMeters < 3.0 * (self.ComputeDescentDistanceMeters() - 0.0001)):
            ...
            if (mach < (self.getTargetCruiseMach() - (self.getTargetCruiseMach() * 0.001))):
                ...
                self.trueAirSpeedAtAltMetersSecond = 0
            deltaDistanceMeters = trueAirSpeedMetersSecond * math.sin(RotorAngleRad) * deltaTimeSeconds
            deltaAltitudeMeters = 0.0
            altitudeMeanSeaLevelMeters += deltaAltitudeMeters
            flightPathAngleDegrees = 0.0
            a = self.ComputeDescentDistanceMeters()
            if (distanceStillToFlyMeters < 3.0 * (self.ComputeDescentDistanceMeters() - 0.0001)):
                self.setDescentConfiguration(elapsedTimeSeconds + deltaTimeSeconds)
            ...
            #zhd 直升机倾斜降落(竖直方向速度增大，方向向下)
            RotorAngle = 30.0
            RotorAngleRad = math.radians(RotorAngle)
            thrustNewtons = thrustNewtons * 7/9
            liftNewtons = thrustNewtons * math.cos(RotorAngleRad)
            ...
            self.trueAirSpeedAtAltMetersSecond += aircraftAccelerationAtAlt * deltaTimeSeconds
            trueAirSpeedMetersSecond -= aircraftAcceleration * deltaTimeSeconds
            if(trueAirSpeedMetersSecond < 0.0001):
                trueAirSpeedMetersSecond = 0.5

            deltaDistanceMeters = trueAirSpeedMetersSecond * math.sin(RotorAngleRad) * deltaTimeSeconds
            deltaAltitudeMeters = self.trueAirSpeedAtAltMetersSecond * deltaTimeSeconds
            HorizontalSpeed = trueAirSpeedMetersSecond * math.sin(RotorAngleRad)
            altitudeMeanSeaLevelMeters += deltaAltitudeMeters
            flightPathAngleDegrees = 3.0
            if(HorizontalSpeed <= 1.0):
            # if (distanceStillToFlyMeters <= self.ComputeDescentDistanceMeters() -  self.ComputeDescentDistanceMeters() * 0.0001):
            ...
            self.trueAirSpeedAtAltMetersSecond += aircraftAccelerationAtAlt * deltaTimeSeconds
            trueAirSpeedMetersSecond = self.trueAirSpeedAtAltMetersSecond

            deltaDistanceMeters = 0.0
            deltaAltitudeMeters = self.trueAirSpeedAtAltMetersSecond * deltaTimeSeconds
            altitudeMeanSeaLevelMeters += deltaAltitudeMeters
            flightPathAngleDegrees = 5.0
            if(self.trueAirSpeedAtAltMetersSecond > 0.0):
                ...
                if (altitudeMeanSeaLevelMeters <= arrivalRunwayTouchDownAltitudeMSLmeters):
                    ...
                    self.trueAirSpeedAtAltMetersSecond = -3.0

            # if (distanceStillToFlyMeters <= self.ComputeDescentDistanceMeters()):
            #     self.setApproachConfiguration(elapsedTimeSeconds + deltaTimeSeconds)
            ...
            #zhd
             ...
             deltaDistanceMeters = trueAirSpeedMetersSecond * deltaTimeSeconds
             deltaAltitudeMeters = 0.0
             if (trueAirSpeedMetersSecond <= (5.0)):
        ...
        #zhd
        fuelFlowKilograms = 0.00002 * deltaTimeSeconds
...
#zhd计算当前高度和目的地高度差
    def ComputeDescentDistanceMeters(self):

        altitudeMeanSeaLevelMeters = self.getCurrentAltitudeSeaLevelMeters()
        approachFixAltitudeMSLmeters = self.arrivalAirportFieldElevationMeters
        deltaAltitudeMeters = altitudeMeanSeaLevelMeters - approachFixAltitudeMSLmeters
